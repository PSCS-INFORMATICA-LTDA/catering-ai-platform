...
describe('final payment flow i18n', () => {
  it('T56-T58 localizes public payment choices in PT EN ES', () => {
    ...
    assert.match(tPayments('pt', 'mileageCourtesyHelp', { n: 20 }), /Até 20 mi/)
    assert.match(tPayments('en', 'mileageCourtesyHelp', { n: 20 }), /Up to 20 mi/)
    assert.match(tPayments('es', 'mileageCourtesyHelp', { n: 20 }), /Hasta 20 mi/)
    ...
    assert.doesNotMatch(tPayments('pt', 'mileageFullTrip'), /franquia/i)
    assert.doesNotMatch(tPayments('en', 'mileageFullTrip'), /allowance/)
    assert.doesNotMatch(tPayments('es', 'mileageFullTrip'), /franquicia/i)
  })
