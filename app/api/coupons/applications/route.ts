...
export async function GET() {
  ...
  const { data: quotes } = quoteIds.length
    ? await db
        .from('quotes')
        .select('id, quote_number, quote_total, customer_id')
        .eq('company_id', ctx.companyId)
        .in('id', quoteIds)
    : { data: [] }
  ...
  const { data: customers } = customerIds.length
    ? await db
        .from('customers')
        .select('id, full_name, ab_name')
        .eq('company_id', ctx.companyId)
        .in('id', customerIds)
    : { data: [] }
  const quoteMap = new Map((quotes ?? []).map((row) => [String(row.id), row]))
  const customerMap = new Map(
    (customers ?? []).map((row) => [String(row.id), row]),
  )
  ...
  return Response.json(
    {
      applications: (applications ?? []).map((application) => {
        ...
        const customer = quote?.customer_id
          ? customerMap.get(String(quote.customer_id))
          : null
        return {
          ...application,
...
export async function PATCH(request: Request) {
  ...
  const { count: invoiceCount, error: invoiceError } = await db
    .from('invoices')
    .select('id', { count: 'exact', head: true })
    .eq('company_id', ctx.companyId)
    .eq('quote_id', application.quote_id)
  if (invoiceError) {
    return Response.json(
  ...
  if ((invoiceCount ?? 0) > 0) {
    return Response.json(
      {
        error:
          'A cotação já possui invoice. Revise o financeiro antes de aprovar o desconto.',
      },
      { status: 409, headers: NO_STORE },
    )
  }
  ...
  const { data: quote, error: quoteError } = await db
    .from('quotes')
    .select('id, pricing_breakdown, quote_total, deposit_amount')
  ...
  if (!breakdown) {
    return Response.json(
      { error: 'Cotação sem snapshot de preço.' },
      { status: 409, headers: NO_STORE },
    )
  }
  ...
  if (!(discount > 0)) {
    return Response.json(
      { error: 'Desconto potencial inválido.' },
      { status: 409, headers: NO_STORE },
    )
  }
  ...
  if (quoteUpdate.error) {
    return Response.json(
      { error: quoteUpdate.error.message },
      { status: 500, headers: NO_STORE },
    )
  }
  ...
  return Response.json(
    { ok: true, status: 'applied', total, balance },
    { headers: NO_STORE },
  )
}
