  ...
  for (const key of KEYS) {
    for (const locale of LOCALES) {
      const entry = new RegExp(
        `"${key.replace('+', '\\+')}":[\\s\\S]{0,400}?"${locale}": "[a-z-]+${locale}-v[3-5]\\.webp"`,
      )
      assert.match(generated, entry, `${key} ${locale} is not mapped`)
    }
  }
  const files = [...generated.matchAll(/"([a-z-]+-v[3-5]\.webp)"/g)].map((m) => m[1])
...
test('ART_FOLLOWS_LOCALE_WITH_FALLBACK', () => {
  ...
  assert.match(visual, /file\.endsWith\('-v5\.webp'\)/)
  assert.match(visual, /\?v=art5/)
  assert.match(generated, /bbqpers-plus-pt-v5\.webp/)
