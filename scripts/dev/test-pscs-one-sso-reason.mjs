...
assert.equal(publicPscsOneSsoReason(new Error('null value in column "company_id" of relation "app_users"')), 'identity_row_rejected')
...
assert.equal(publicPscsOneSsoReason(new Error('column app_users.auth_user_id does not exist')), 'identity_schema_mismatch')
assert.equal(publicPscsOneSsoReason(new Error('fetch failed')), 'session_fetch_failed')
assert.equal(publicPscsOneSsoReason(new Error('Database error saving new user')), 'identity_db_denied')
