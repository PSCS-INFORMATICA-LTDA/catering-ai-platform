...
- O domínio DEV canônico deve permanecer com `gitBranch=feat/public-self-service-quote-dev` e `autoAssignCustomDomains=false`. Isso **não impede** `vercel deploy --prod` de roubar `catering-ai-agenda-dev.vercel.app`: esse host é alias `.vercel.app` do projeto, não custom domain. `gitBranch` sozinho também NÃO recupera o alias depois que um CLI `--prod` (sem SHA) já o estiver segurando. Depois de cada push em `feat/public-self-service-quote-dev`, e imediatamente se `/quote` voltar a pedir login, executar `npm run bind:dev:canonical-alias` e `npm run verify:dev:public-quote`.
